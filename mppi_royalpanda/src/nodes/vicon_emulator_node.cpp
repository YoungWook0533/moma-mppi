...
int main(int argc, char** argv) {
  ...
  nh.param<std::string>("link_name", link_name, "reference_link_gazebo");
  ...
  auto cb = [&](const gazebo_msgs::LinkStatesConstPtr& msg) {
    ...
    if (iter == msg->name.end()) {
      ROS_WARN_STREAM_THROTTLE(2.0, "Link: " << link_name << " not found in gazebo link states.");
      return;
    }

    size_t idx = iter - msg->name.begin();
    base_odometry.header.stamp = ros::Time::now();  // Use current ROS time
  ...
  ros::Subscriber link_subscriber = nh.subscribe<gazebo_msgs::LinkStates>("/gazebo/link_states", 10, cb);
