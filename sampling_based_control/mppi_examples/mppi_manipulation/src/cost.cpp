...
//     Eigen::Vector3d fr3_link0 = robot_model.get_pose("fr3_link0").translation;
//     Eigen::Vector3d fr3_link2 = robot_model.get_pose("fr3_link2").translation;
...
//     FG.point1 = fr3_link2;
//     FG.point2 = robot_model.get_pose("fr3_link3").translation;
//     FG.radius = 0.07;
//     ssvs.push_back(FG);

//     // Segment GH (Link 4 to Link 5)
//     GH.point1 = robot_model.get_pose("fr3_link4").translation;
//     GH.point2 = Eigen::Vector3d(0, 0.025, 0) + robot_model.get_pose("fr3_link5").translation;
//     GH.radius = 0.09;
//     ssvs.push_back(GH);
...
mppi::cost_t PandaCost::compute_cost(const mppi::observation_t& x,
  ...
  // regularization cost
  cost += 1.0 * x.segment<3>(0).norm() + 1.0 * x.segment<7>(3).norm();
  
  // end effector reaching cost
  Eigen::Vector3d ref_t = ref.head<3>();
  Eigen::Quaterniond ref_q(ref.segment<4>(3));
  robot_model_.get_error("fr3_hand_tcp", ref_q, ref_t, error_);
  cost +=
      (error_.head<3>().transpose() * error_.head<3>()).norm() * params_.Qt * 300;
  cost +=
      (error_.tail<3>().transpose() * error_.tail<3>()).norm() * params_.Qr * 300;

  // ROS_WARN("err : %f %f %f", error_(0), error_(1), error_(2));
